...
function image_fusion(image_1,image_2,solution,savePath)
...
%writematrix(solution_1*solution, '1to2.csv');
csvwrite('1to2.csv', solution_1*solution);
...
[parent_path, ~, ext] = fileparts(savePath);
imwrite(fusion_image,strcat(parent_path,'\chessboard',ext));
close(gcf);
...
str=['.\test_imgs\','Fusedimageoftheboard.jpg'];

[parent_path, ~, ext] = fileparts(savePath);
imwrite(f_3,strcat(parent_path,'\chessboard',ext));
close(gcf);
