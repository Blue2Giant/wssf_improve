...
function cp_showMatch(I1,I2,loc1,loc2,correctPos,savePath)
%% 首先确保图像都是三通道影像
[~,~,num_1] =size(I1);
[~,~,num_2] =size(I2);
if num_1>3
...
figure,imshow(im3,[]);
imwrite(im3,'mathing_result','jpg')
% title(tname);
hold on
...
saveas(gcf,strcat(parent_path,'\matched_features',ext)); % 保存为 PNG 格式，你可以根据需要修改文件名和格式
close(gcf);
